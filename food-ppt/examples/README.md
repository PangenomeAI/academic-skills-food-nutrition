...
Built with the **sage** theme (one of 17 in `../templates/INDEX.md`; preview
...
## ai-news-brief-demo (dark `midnight` theme)
A 9-slide briefing that shows the **`midnight`** dark theme (dark slate + blue), plus the
`cards`, `two_column`, and `references` layouts.
- `ai-news-brief-demo.deck.json` / `.pptx`.
- **Recreated from a user-supplied deck generated by Grok 4.5** ("Major AI News — Past
  7 Days"), rebuilt with food-ppt's editable layouts to demonstrate the dark theme and
  how its style is reproduced natively (no file copied; the palette was re-derived).
- **Content is the source deck's** and is reproduced for **layout demonstration only —
  it is not independently verified** (AI-generated news, some items speculative).

Rebuild: `python3 ../scripts/build_pptx.py ai-news-brief-demo.deck.json --out ai-news-brief-demo.pptx`.
